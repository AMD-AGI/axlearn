...
if [[ $TUNED_GEMMS -eq 1 ]]; then
    if [[ ! -f "$tuned_gemms_path" ]]; then
        echo "No tuned GEMMs for $(build_mesh_string) found in path ${tuned_gemms_path}! you can still run training by setting TUNED_GEMMS=0"
        export XLA_FLAGS=$XLA_FLAGS" --xla_gpu_autotune_level=4" # auto-tuning at XLA level if not using hipblaslt tuned GEMMs
        exit 0
    else
        export HIPBLASLT_TUNING_OVERRIDE_FILE="$tuned_gemms_path"
        export XLA_FLAGS=$XLA_FLAGS" --xla_gpu_autotune_level=0" # no XLA auto-tuning if hipblaslt
    fi
else
